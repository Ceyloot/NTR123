/**
 * Prompt for point-based object detection.
 */
export const POINT_DETECTION_PROMPT = (x: number, y: number) => `
You are a highly precise visual perception assistant.
I am providing an image and a specific coordinate: X=${Math.round(x * 100)}%, Y=${Math.round(y * 100)}%.
...
export const QUANTITY_ANALYSIS_GEMINI_INSTRUCTION = `You are a visuo-linguistic analyst specializing in Polish-English context.
...
Output ONLY "singular" or "plural" (lowercase).`;

export const BLUEPRINT_ANALYSIS_GEMINI_INSTRUCTION = `You are a visual blueprint analyst.
...
Output ONLY the descriptive text.`;
